...
namespace com.IvanMurzak.McpPlugin.Tests.Network.Connection
{
    ...
    public class ConnectionManagerRejectionTests
    {
        ...
        private readonly Mock<IWebSocketConnectionProvider> _mockProvider;
        ...
        public ConnectionManagerRejectionTests(ITestOutputHelper output)
        {
            ...
            _mockProvider = new Mock<IWebSocketConnectionProvider>();
            _testVersion = new Common.Version { Api = "1.0.0", Plugin = "1.0.0", Environment = "test" };
            _testEndpoint = "http://localhost:5000/hub";

            _mockProvider.SetupGet(x => x.JsonSerializerOptions).Returns(new JsonSerializerOptions());

            _mockProvider
                .Setup(x => x.CreateConnectionAsync(It.IsAny<string>()))
                .ReturnsAsync(CreateDummyConnection());
        }
        ...
        private static (System.Net.WebSockets.ClientWebSocket, Uri) CreateDummyConnection()
        {
            return (new System.Net.WebSockets.ClientWebSocket(), new Uri("ws://localhost:9999/dummy"));
        }
        ...
        private abstract class FastConnectionManager : ConnectionManager
        {
            ...
            protected FastConnectionManager(
                ILogger logger, Common.Version version, string endpoint, IWebSocketConnectionProvider provider)
        ...
        private class SequenceConnectionManager : FastConnectionManager
        {
            ...
            public SequenceConnectionManager(
                ...
                IWebSocketConnectionProvider provider, bool[] attemptResults)
                : base(logger, version, endpoint, provider)
            {
