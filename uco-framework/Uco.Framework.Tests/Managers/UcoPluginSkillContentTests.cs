...
namespace com.AtelierAI.Uco.Framework.Tests.Managers
{
    ...
    public class UcoPluginSkillContentTests : IDisposable
    {
        ...
        public UcoPluginSkillContentTests(ITestOutputHelper output)
        {
            _output = output;
            _loggerProvider = new XunitTestOutputLoggerProvider(output);
            _tempDir = Path.Combine(Path.GetTempPath(), "UcoPluginSkillContentTests_" + Guid.NewGuid().ToString("N"));
