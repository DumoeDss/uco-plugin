...
namespace com.AtelierAI.Uco.Framework.Tests.Managers
{
    ...
    public class UcoBuilderTests_ListTool
    {
        ...
        IUcoPlugin? BuildUcoPluginWithTool(string toolName, string toolTitle)
        {
            // Arrange
            var classType = typeof(Method_NoArgs_Void);
            var method = typeof(Method_NoArgs_Void).GetMethod(nameof(Method_NoArgs_Void.Do))!;
            var reflector = new Reflector();
            var ucoPluginBuilder = new UcoBuilder(version, _loggerProvider)
        ...
        async Task BuildAndValidateTool(Type classType, MethodInfo method, JsonElement? expectedInputSchema = null, JsonElement? expectedOutputSchema = null)
        {
            ...
            var listToolTask = ucoPlugin.UcoManager.ToolManager!.RunListTool(request);
