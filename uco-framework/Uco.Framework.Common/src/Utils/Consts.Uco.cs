...
namespace com.AtelierAI.Uco.Framework.Common
{
    ...
    {
        public static partial class Uco
        {
            ...
            public static partial class Server
            {
                ...
                /// <c>HttpServerTransportOptions.IdleTimeout</c>. An idle Uco session is evicted
                ...
                /// <summary>
                /// HTTP request headers recognised by the Uco plugin server beyond
                ...
                public static class Headers
                {
                    ...
                    /// to <c>"1"</c>, the Uco list endpoints (<c>tools/list</c>,
                    ...
                    /// </summary>
                    public const string TrustedInternalClient = "X-UcoPlugin-Internal-Client";
                    ...
                    /// <summary>
                    /// Per-client session correlation header. When the client sends
                    /// this header, the server uses its value as the logical session
                    /// id for the request (drives per-session tool gating, per-session
                    /// active-instance routing, etc.). When the header is absent on
                    /// an HTTP request, the middleware generates a fresh id and echoes
                    /// it back via the response of the same name — the client is
                    /// expected to send the echoed value on every subsequent request.
                    /// stdio transport uses a fixed sentinel session id since there
                    /// is no HTTP request.
                    /// </summary>
                    public const string UcoSessionId = "Uco-Session-Id";

                    /// <summary>
                    /// Sentinel session id used by the stdio transport (which has no
                    /// HTTP headers). All stdio requests share this implicit session.
                    /// </summary>
                    public const string StdioSessionId = "__stdio__";

                    /// <summary>
                    /// Per-connection plugin instance id advertised by SignalR plugin
                    /// clients at connect time. The server reads it from the SignalR
                    /// negotiate request (URL query <c>instanceId</c> takes precedence;
                    /// this header is the fallback for transports that rewrite the URL
                    /// on upgrade) and registers the connection in its instance
