...
namespace com.AtelierAI.Uco.Framework
{
    ...
    public class UcoManagerClientHub : BaseHubConnector, IPluginManagerHub
    {
        readonly IClientPluginManager _pluginManager;
        ...
        public UcoManagerClientHub(
            ...
            IClientPluginManager pluginManager,
        ...
        protected override void RegisterServerHandlers(IConnectionManager connectionManager, CompositeDisposable disposables)
        {
            ...
            connectionManager.RegisterNotification<OnInitialClientDataParams>(
                nameof(IClientRpc.OnInitialClientData),
                async param =>
                {
                    _logger.LogDebug("{class}.{method}", nameof(IClientRpc), nameof(IClientRpc.OnInitialClientData));
                    if (_liveNotificationEpoch != 0)
                    {
                        _logger.LogDebug("{class}.{method} Discarding stale initial snapshot: live notifications already received.",
                            nameof(UcoManagerClientHub), nameof(IClientRpc.OnInitialClientData));
                        return;
                    }
                    if (param != null)
                        await _pluginManager.OnInitialClientData(param.Clients);
                })
                .AddTo(disposables);

            connectionManager.RegisterNotification<OnPluginClientConnectedParams>(
                nameof(IClientRpc.OnPluginClientConnected),
                async param =>
                {
                    _logger.LogDebug("{class}.{method}", nameof(IClientRpc), nameof(IClientRpc.OnPluginClientConnected));
                    Interlocked.Increment(ref _liveNotificationEpoch);
                    if (param != null)
                        await _pluginManager.OnPluginClientConnected(param.Connected, param.All);
                })
                .AddTo(disposables);

            connectionManager.RegisterNotification<OnPluginClientDisconnectedParams>(
                nameof(IClientRpc.OnPluginClientDisconnected),
                async param =>
                {
                    _logger.LogDebug("{class}.{method}", nameof(IClientRpc), nameof(IClientRpc.OnPluginClientDisconnected));
                    Interlocked.Increment(ref _liveNotificationEpoch);
                    if (param != null)
                        await _pluginManager.OnPluginClientDisconnected(param.Disconnected, param.Remaining);
                })
                .AddTo(disposables);

            connectionManager.RegisterNotification<ForceDisconnectParams>(
                nameof(IClientRpc.ForceDisconnect),
                async param =>
                {
                    _logger.LogDebug("{class}.{method}", nameof(IClientRpc), nameof(IClientRpc.ForceDisconnect));
                    ...
                    if (connectionManager.ConnectionGeneration != expectedGeneration)
                    {
                        _logger.LogWarning("{class}.{method} Received ForceDisconnect on a stale connection — ignoring to protect the active connection.",
                            nameof(UcoManagerClientHub), nameof(IClientRpc.ForceDisconnect));
                        return;
                    }
                    ...
                    await _pluginManager.ForceDisconnect();
            ...
            if (_pluginManager.ToolHub != null)
            {
                connectionManager.RegisterHandler<RequestCallTool, ResponseData<ResponseCallTool>>(
                    ...
                    (data, requestCancellationToken) =>
                    {
                        ...
                        return _pluginManager.ToolHub!.RunCallTool(data!, requestCancellationToken);
                    })
                    .AddTo(disposables);
                ...
                connectionManager.RegisterHandler<RequestListTool, ResponseData<ResponseListTool[]>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.ToolHub!.RunListTool(data!);
                    })
                    .AddTo(disposables);
            }
            ...
            if (_pluginManager.PromptHub != null)
            {
                connectionManager.RegisterHandler<RequestGetPrompt, ResponseData<ResponseGetPrompt>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.PromptHub!.RunGetPrompt(data!);
                    })
                    .AddTo(disposables);
                ...
                connectionManager.RegisterHandler<RequestListPrompts, ResponseData<ResponseListPrompts>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.PromptHub!.RunListPrompts(data!);
                    })
                    .AddTo(disposables);
            }
            ...
            if (_pluginManager.ResourceHub != null)
            {
                connectionManager.RegisterHandler<RequestResourceContent, ResponseData<ResponseResourceContent[]>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.ResourceHub!.RunResourceContent(data!);
                    })
                    .AddTo(disposables);
                ...
                connectionManager.RegisterHandler<RequestListResources, ResponseData<ResponseListResource[]>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.ResourceHub!.RunListResources(data!);
                    })
                    .AddTo(disposables);
                ...
                connectionManager.RegisterHandler<RequestListResourceTemplates, ResponseData<ResponseResourceTemplate[]>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.ResourceHub!.RunResourceTemplates(data!);
                    })
                    .AddTo(disposables);
            }
            ...
            if (_pluginManager.SystemToolHub != null)
            {
                connectionManager.RegisterHandler<RequestCallTool, ResponseData<ResponseCallTool>>(
                    ...
                    (data, requestCancellationToken) =>
                    {
                        ...
                        return _pluginManager.SystemToolHub!.RunSystemTool(data!, requestCancellationToken);
                    })
                    .AddTo(disposables);
                ...
                connectionManager.RegisterHandler<RequestListTool, ResponseData<ResponseListTool[]>>(
                    ...
                    data =>
                    {
                        ...
                        return _pluginManager.SystemToolHub!.RunListSystemTool(data!);
                    })
                    .AddTo(disposables);
            }
        }
        ...
        {
            _logger.LogTrace("{class}.{method}", nameof(IServerPluginManager), nameof(IServerPluginManager.NotifyAboutUpdatedTools));
            return _connectionManager.InvokeAsync<RequestToolsUpdated, ResponseData>(nameof(IServerPluginManager.NotifyAboutUpdatedTools), request, cancellationToken);
        }
        ...
        {
            _logger.LogTrace("{class}.{method}", nameof(IServerPluginManager), nameof(IServerPluginManager.NotifyAboutUpdatedPrompts));
            return _connectionManager.InvokeAsync<RequestPromptsUpdated, ResponseData>(nameof(IServerPluginManager.NotifyAboutUpdatedPrompts), request, cancellationToken);
        }
        ...
        {
            _logger.LogTrace("{class}.{method}", nameof(IServerPluginManager), nameof(IServerPluginManager.NotifyAboutUpdatedResources));
            return _connectionManager.InvokeAsync<RequestResourcesUpdated, ResponseData>(nameof(IServerPluginManager.NotifyAboutUpdatedResources), request, cancellationToken);
        }
        ...
        public Task<ResponseData> NotifyToolRequestCompleted(RequestToolCompletedData request, CancellationToken cancellationToken = default)
        {
            if (_logger.IsEnabled(LogLevel.Trace))
            {
                _logger.LogTrace("{class}.{method} request: {RequestId}\n{Json}",
                    nameof(IServerPluginManager),
                    nameof(IServerPluginManager.NotifyToolRequestCompleted),
                    request.RequestId,
                    request.ToPrettyJson()
                );
            }
            return _connectionManager.InvokeAsync<RequestToolCompletedData, ResponseData>(nameof(IServerPluginManager.NotifyToolRequestCompleted), request, cancellationToken);
        }

        public Task<UcoClientData[]> GetPluginClientData()
        {
            _logger.LogTrace("{class}.{method}", nameof(IServerPluginManager), nameof(IServerPluginManager.GetPluginClientData));
            return _connectionManager.InvokeAsync<UcoClientData[]>(nameof(IServerPluginManager.GetPluginClientData), _cancellationTokenSource.Token);
        }

        public Task<UcoServerData> GetServerData()
        {
            _logger.LogTrace("{class}.{method}", nameof(IServerPluginManager), nameof(IServerPluginManager.GetServerData));
            return _connectionManager.InvokeAsync<UcoServerData>(nameof(IServerPluginManager.GetServerData), _cancellationTokenSource.Token);
        }
