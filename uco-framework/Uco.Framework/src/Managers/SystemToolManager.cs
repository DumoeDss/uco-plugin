...
{
    /// <summary>
    /// Manages system tools — internal tools available via HTTP API but NOT exposed to Uco clients.
                ...
                var result = _tools.Keys.ToArray()
                    ...
                    .Select(tool =>
                    {
                        var response = new ResponseListTool()
                        {
                            ...
                            InputSchema = tool.InputSchema.ToJsonElement() ?? Common.Consts.Uco.EmptyInputSchema,
