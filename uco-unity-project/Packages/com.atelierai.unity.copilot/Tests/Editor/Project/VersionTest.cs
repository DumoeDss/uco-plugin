...
namespace com.AtelierAI.Unity.Copilot.Editor.Tests
{
    ...
    {
        private const string PackageName = "com.atelierai.unity.copilot";
        ...
        public IEnumerator Version_Should_Match_PackageJson_Version()
        {
            ...
            // Fallback: try to read from Packages/com.atelierai.unity.copilot/package.json (for source project testing)
            if (string.IsNullOrEmpty(packageJsonContent))
            {
                var fallbackPath = Path.Combine(Application.dataPath, "..", "Packages", "com.atelierai.unity.copilot", "package.json");
            ...
            Assert.IsFalse(string.IsNullOrEmpty(packageJsonContent),
                $"package.json not found. Tried PackageManager location and fallback path: Packages/com.atelierai.unity.copilot/package.json");
            ...
            // Compare versions
            var pluginVersion = UnityCopilotPlugin.Version;
