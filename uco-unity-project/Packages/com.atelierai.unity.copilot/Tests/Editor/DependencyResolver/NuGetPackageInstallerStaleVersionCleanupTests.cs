    ...
    public class NuGetPackageInstallerStaleVersionCleanupTests
    {
        ...
        public void SetUp()
        {
            _installPath = Path.Combine(
                Path.GetTempPath(),
                "Uco-NuGetInstaller-" + Path.GetRandomFileName());
        ...
        public void RemoveStaleSiblingVersions_PreservesOtherPackages()
        {
            ...
            Assert.IsTrue(manifest.Packages.ContainsKey("R3"),
        ...
        {
            CreateFlatPackage("Microsoft.Extensions.Logging.Abstractions", "10.0.3",
                "Microsoft.Extensions.Logging.Abstractions.10.0.3.dll");

            var removed = NuGetPackageInstaller.RemoveStaleSiblingVersions(
                _installPath, "Microsoft.Extensions.Logging.Abstractions", "8.0.3");

            Assert.IsTrue(removed);
            var manifest = NuGetInstallManifest.Load(_installPath);
            Assert.IsFalse(manifest.Packages.ContainsKey("Microsoft.Extensions.Logging.Abstractions"));
        }
