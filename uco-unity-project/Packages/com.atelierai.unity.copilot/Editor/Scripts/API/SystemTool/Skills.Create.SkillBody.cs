...
Prefer returning a structured data model over a plain string so the AI can parse individual fields. Data models for Uco tools MUST be declared as TOP-LEVEL types in the `AIGD` namespace - never nested inside the tool class. Place each data model in its own `.cs` file (one type per file) and use `ResponseCallValueTool<T>` as the return type. The flat `AIGD` namespace keeps the auto-generated JSON Schema `$defs` keys short and intuitive for AI agents.
```csharp
...
```
For simpler cases that do not need async/processing, you may return the model directly (without `ResponseCallValueTool<T>`) and Unity-Uco will wrap it automatically. The data model itself MUST still live as a top-level type in the `AIGD` namespace.
