    ...
    public class ExtensionPanel
    {
        ...
        private async Task InstallOrUpdateAsync()
        {
            ...
            try
            {
                ...
                if (version == null)
                {
                    Debug.LogError($"[Unity Copilot] Failed to fetch latest version for '{_data.Name}' from OpenUPM.");
                ...
                Debug.Log($"[Unity Copilot] Extension '{_data.Name}' ({_data.PackageId}@{version}) added to manifest.json. Resolving packages...");
                Client.Resolve();
            }
            catch (Exception ex)
            {
                Debug.LogError($"[Unity Copilot] Failed to {(isUpdate ? "update" : "install")} extension '{_data.Name}': {ex.Message}");
                ResetButton(isUpdate);
            }
        }
        ...
        internal static bool AddToManifest(string manifestPath, string packageId, string version)
        {
            ...
            {
                Debug.LogError($"[Unity Copilot] {manifestPath} not found.");
                return false;
            }
            ...
            if (manifest == null)
            {
                Debug.LogError($"[Unity Copilot] Failed to parse {manifestPath} as JSON.");
                return false;
            }
        ...
        private static async Task<string?> FetchLatestOpenUpmVersionAsync(string packageId)
        {
            ...
            client.DefaultRequestHeaders.Add("User-Agent", "Unity-Copilot-ExtensionPanel");
