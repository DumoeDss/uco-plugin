...
namespace com.AtelierAI.Unity.Copilot.Editor.UI
{
    ...
    public class UpdatePopupWindow : NotificationPopupWindow
    {
        ...
        private const string PackageId = "com.atelierai.unity.copilot";
        ...
        private void OnInstallUpdateClicked()
        {
            ...
            var currentStatus = CopilotServerManager.ServerStatus.CurrentValue;
            if (currentStatus == CopilotServerStatus.Running || currentStatus == CopilotServerStatus.Starting ||
                currentStatus == CopilotServerStatus.Stopping)
            {
                installButton.text = "Stopping server...";
                if (currentStatus != CopilotServerStatus.Stopping)
                    CopilotServerManager.StopServer();
                _stopSubscription = CopilotServerManager.ServerStatus
        ...
        private void OnPackageInstallProgress()
        {
            ...
            else if (addRequest.Status >= StatusCode.Failure)
            {
                ...
                UnityCopilotPluginEditor.Instance.LogError("Failed to update package: {error}", typeof(UpdatePopupWindow), errorMessage);
                EditorUtility.DisplayDialog(
                    "Update Failed",
