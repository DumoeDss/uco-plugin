        ...
        )
        {
            ...
            Func<SerializedMember> action = () =>
            {
                ...
                var dictInputParameters = inputParameters?.ToDictionary(
                    ...
                    elementSelector: p => reflector.Deserialize(p, logger: UnityCopilotPluginEditor.Instance.Logger)
                );
                ...
                if (string.IsNullOrEmpty(targetObject?.typeName))
                {
                    ...
                    methodWrapper = new MethodWrapper(reflector, logger: UnityCopilotPluginEditor.Instance.Logger, method);
                }
                else
                {
                    // Object instance needed. Probably instance method.
                    var obj = reflector.Deserialize(targetObject!, logger: UnityCopilotPluginEditor.Instance.Logger);
                    if (obj == null)
                    ...
                    methodWrapper = new MethodWrapper(
                        reflector: reflector,
                        logger: UnityCopilotPluginEditor.Instance.Logger,
                        targetInstance: obj,
                        methodInfo: method);
                }
