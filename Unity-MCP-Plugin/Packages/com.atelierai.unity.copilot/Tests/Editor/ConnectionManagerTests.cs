...
namespace com.AtelierAI.Unity.Copilot.Editor.Tests
{
    ...
    public class ConnectionManagerTests
    {
        ...
        public void ConnectionManager_Constructor_ShouldInitializeCorrectly()
        {
            _connectionManager = new ConnectionManager(
                ...
                wsProvider: _mockConnectionProvider);

            // Assert
            Assert.IsNotNull(_connectionManager, "ConnectionManager should be initialized");
            Assert.AreEqual(ConnectionState.Disconnected, _connectionManager.ConnectionState.CurrentValue,
    ...
    public class MockHubEndpointConnectionProvider : IWebSocketConnectionProvider
    {
        public bool ShouldFailConnection { get; set; } = false;

        public JsonSerializerOptions JsonSerializerOptions { get; } = new JsonSerializerOptions();

        public async Task<(ClientWebSocket WebSocket, Uri Uri)> CreateConnectionAsync(string endpoint)
        {
            ...
            // Real implementation would create a ClientWebSocket
            return (null!, null!);
        }
    }
}